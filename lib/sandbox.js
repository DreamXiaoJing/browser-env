...
// 不注入核心内置构造函数（Object/Array/Function/String/Number/Boolean/Symbol/Date/
// Map/Set/WeakMap/WeakSet/Promise/Proxy/Reflect/Error 系列/TypedArray/DataView/JSON/Math/Intl/
// 全局函数如 parseInt 等），因为 vm.createContext 会自动提供它们，且必须使用 context 内置
// 版本——否则 primitive 的原型链查询（如 "abc".__proto__ === String.prototype）会失效，
// 因为 primitive 的 [[Get]] 用的是 context 内置的 String，而非注入的主上下文 String。
//
// 仅注入 Node.js 全局可用、但 vm context 默认缺失的 Web API。
...
function createSandbox(config = {}) {
  ...
  // 2. 创建 vm 上下文（内置构造函数由 context 自身提供）
  const ctx = vm.createContext(sandbox);

  // 2.5. 确保 eval 作为全局对象属性可用（vm.createContext 不自动添加）
  //     RS6 VMP 通过 window.eval / window.execScript 访问 eval
  //     注意：context 内部 this.eval 是内置 eval，但外部访问 ctx.eval 拿不到，
  //     所以必须显式赋值一个可枚举/可访问的 eval 引用。
  vm.runInContext(`
    (function() {
      var nativeEval = eval;
      this.eval = function(s) { return nativeEval(s); };
      this.eval.toString = function() { return 'function eval() { [native code] }'; };
  ...
  // 包装 console.error：过滤掉阿里云验证码 FeiLin 库在 Promise.race(...).catch()
  // 中故意打印的非致命错误。这些错误来自验证码图片预加载逻辑——由于沙箱 DOM
  // 不包含验证码图片元素，to() 函数中 n.onload 赋值会抛 "Cannot set properties
  // of null (setting 'onload')"，FeiLin 的 catch 回调用 console.error(t) 打印。
  // 该错误不影响验证码流程（InitCaptchaV3/Log2/Log3 请求均正常发送）。
  var _origErr = consoleObj.error;
  consoleObj.error = function() {
    var a0 = arguments[0];
    if (a0 && typeof a0 === 'object' && typeof a0.message === 'string' &&
        a0.message.indexOf("Cannot set properties of null (setting 'onload')") !== -1) {
      return; // 静默吞掉这个非致命错误
    }
    return _origErr.apply(this, arguments);
  };
  ctx.console = consoleObj;

  // 11. 抑制沙箱内 Promise 产生的 unhandledRejection（动态加载的 JS 如 FeiLin
  //     反爬检测会在 Promise 链里抛错，不抑制会导致进程卡死或刷屏）
  if (!process._sandboxRejectionHooked) {
    process._sandboxRejectionHooked = true;
    process.on('unhandledRejection', function() { /* swallow */ });
  }
